...
def test_normal_with_parallel_only_deepseek():
    ...
    assert "qwen3-max" not in models
...
def test_deep_flash_with_parallel_only_deepseek():
    ...
    models = [m.model for m in result]
    assert "deepseek-v4-flash" in models
    assert "glm-4.5-flash" not in models
...
def test_is_pinned_model_eligible():
    ...
    assert is_pinned_model_eligible("deepseek-v4-flash", eligible) is True
    assert is_pinned_model_eligible("qwen3-max", eligible) is False
