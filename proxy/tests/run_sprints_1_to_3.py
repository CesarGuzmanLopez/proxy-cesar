...
async def simple_chat(model: str, msg: str = "say hi", max_tokens: int = 300, conv_id: str = None):
    """POST no-streaming."""
...
async def test_sprint1():
    ...
    for m in NON_VISION:
        ...
        log_test(f"  {m} streaming", ok and done, f"chunks={n} content_in_chunks={ok} done={done}")
    ...
    log_test("  2o request = misma physical", phys1 == phys2, f"phys1={phys1} phys2={phys2}")
    ...
    if st == 200:
        ...
        required = ["physical_model", "pseudo_model", "conversation_id",
                     "affinity_maintained", "fallback_applied", "context_tokens_total"]
        present = all(k in meta for k in required)
        log_test("  Todos los campos S1 en metadata", present,
...
async def test_sprint2():
    ...
    if st == 200:
        ...
        log_test("  capabilities_detected presente", has_has_images and has_has_tools,
    ...
    async with httpx.AsyncClient(timeout=30) as c:
        ...
        log_test("  Exceso de threshold → 400", ok,
                 f"HTTP {r.status_code} error={detail}")
    ...
    if st == 200:
        meta = d.get("proxy_metadata", {})
        log_test("  compactador responde", True,
                 f"phys={meta.get('physical_model','?')}")
    else:
    ...
    async with httpx.AsyncClient() as c:
        ...
        if r.status_code == 200:
            ...
            expected = {"pensamiento-profundo-caro", "tareas-avanzadas",
                        "avanzada-vision", "normal", "deep-flash",
                        "flash-lowcost", "flash-vision", "compactador"}
            missing = expected - ids
            log_test("  Todos los pseudo-models listados",
                     len(missing) == 0, f"missing={missing}")
        else:
            log_test("  Todos los pseudo-models listados", False, f"HTTP {r.status_code}")
...
async def test_sprint3():
    ...
    if st == 200:
        meta = d.get("proxy_metadata", {})
        log_test("  tools_filter_applied=false (sin tools previas)",
    ...
    st, d = await simple_chat("pensamiento-profundo-caro",
                               "solve 23+45 step by step", 500)
    if st == 200:
        ...
        log_test("  Respuesta con razonamiento",
                 bool(content),
    ...
    if st == 200:
        meta = d.get("proxy_metadata", {})
        log_test("  fallback_applied false en éxito",
...
async def test_context():
    ...
    st1, _ = await simple_chat("flash-lowcost",
                                f"my secret code is {secret} remember it", 500, conv)
    await asyncio.sleep(0.5)
    ...
    async with httpx.AsyncClient(timeout=120) as c:
        ...
        if r.status_code == 200:
            ctext = r.json().get("choices", [{}])[0].get("message", {}).get("content", "").lower()
            remembered = secret.lower() in ctext
            log_test("  Contexto preservado entre turns",
                     remembered,
