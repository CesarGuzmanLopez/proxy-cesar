# Proxy Determinista Multi-Modelo
...
POST /v1/chat/completions
  ...
  │
  ├─ 4. Si hay tools: filtrar pool de modelos a solo OpenAI-compatible-tools
  │     Si el modelo fijado no califica → error TOOLS_MODEL_INCOMPATIBLE
  │
...
- El campo `openai_tools_compatible` determina si el modelo entra al pool cuando la conversación usa tools. Si es `false`, ese modelo queda excluido en conversaciones con tools — sin excepción.
...
pensamiento-profundo-caro:
  ...
  physical_models:
    ...
    - provider: anthropic
      ...
      parallel_tools: true
      vision: true
      note: "Solo como último fallback. Costoso."
  fallback_strategy: sequential
```

**Notas de diseño:**

- Prioridad: GLM 5.1 → o3 → Claude Opus (por costo)
...
tareas-avanzadas:
  ...
  physical_models:
    ...
    - provider: deepseek
      ...
      parallel_tools: true
      vision: false
      note: "Alternativa con razonamiento explícito"
    - provider: minimax
      model: minimax-m2.5
      openai_tools_compatible: false # No confiable con tools
      tools_strict: false
      parallel_tools: false
      vision: false
      note: "Solo para conversaciones sin tools"
  fallback_strategy: sequential
```

**Notas de diseño:**

- MiniMax M2.5 está en la lista pero tiene `openai_tools_compatible: false`. Cuando la conversación usa tools, el proxy lo excluye automáticamente del pool. Solo entra como fallback en conversaciones sin tools.
- DeepSeek V3 Pro soporta `strict: true` → schemas de tools garantizados.
...
flash-lowcost:
  ...
  physical_models:
    ...
    - provider: ollama
      model: llama3.2
      openai_tools_compatible: false # local, sin soporte confiable de tools
      tools_strict: false
      parallel_tools: false
      vision: false
      note: "Fallback local. Sin tools."
  fallback_strategy: sequential
```
...
compactador:
  ...
  physical_models:
    - provider: google
      ...
      openai_tools_compatible: false # no necesita tools para compactar
...
Cuando una conversación usa `normal` → `qwen-3.6`, el proveedor cachea el prefijo (system + tools + historial). Si en el turno 5 el proxy envía la conversación a `deepseek-v3-flash`, el caché de Qwen se destruye: DeepSeek comienza desde cero, el costo aumenta, la latencia aumenta.
...
Turno 1: conversación "abc", pseudo-modelo "normal"
  → proxy lee pseudo_models.yaml → primer modelo: "qwen-3.6"
  → Valkey: SET conv:abc:physical_model "qwen-3.6" EX 86400
  → PostgreSQL: conversation.physical_model = "qwen-3.6"

Turnos 2-50: mismo pseudo-modelo
  → proxy: GET conv:abc:physical_model → "qwen-3.6"
  → SIEMPRE usa "qwen-3.6" (valor exacto del config, sin modificar)
...
Fallback en turno 20: "qwen-3.6" retorna 503
  → proxy intenta "deepseek-v3-flash" (prioridad 2)
  → Valkey: actualizar afinidad a "deepseek-v3-flash"
...
**Dentro de los modelos elegibles, se prefiere `tools_strict: true`** cuando el proveedor lo soporta (DeepSeek V3, OpenAI). Esto garantiza que el JSON de argumentos siempre respeta el schema, eliminando alucinaciones de parámetros.
...
| -------------------- | ------------------------- | -------------- | ---------------- | ------------------------------------------- |
| Claude Opus / Sonnet | ✅ (vía LiteLLM)          | ✗              | ✅               | Robusto. Formato propio → LiteLLM traduce   |
| GPT-4o / o3          | ✅ nativo                 | ✅             | ✅               | Referencia de formato                       |
| Gemini 2.5 Flash     | ✅ (vía LiteLLM)          | ✗              | Parcial          | LiteLLM traduce. Parallel limitado          |
| DeepSeek V3 / V3 Pro | ✅ nativo                 | ✅             | ✅               | OpenAI-compatible, strict soportado         |
| DeepSeek R1          | ✅ nativo                 | ✗              | ✅               | Compatible pero sin strict                  |
| Qwen 3.6 / 2.5-72B   | ✅ nativo                 | ✗              | ✗                | Compatible, parallel no confiable           |
| GLM-5.1              | ✅ nativo                 | ✗              | ✗                | Funciona en casos no parallel               |
| GLM-4.7-Flash        | ✅ nativo                 | ✗              | ✗                | Básico. Solo tools simples y bien definidas |
| Groq 20B (Llama 3.3) | ✅ nativo                 | ✗              | ✗                | Compatible, parallel no confiable           |
| MiniMax M2.5         | ❌                        | ✗              | ✗                | Excluido de conversaciones con tools        |
| MiniMax M2.7 Vision  | ❌                        | ✗              | ✗                | Excluido de conversaciones con tools        |
| Qwen 3.5-9B          | ✅ nativo                 | ✗              | ✗                | Básico. Úsese con schemas simples           |
| LLaVA / Ollama local | ❌                        | ✗              | ✗                | Sin soporte confiable                       |
| Llama3.2 local       | ❌                        | ✗              | ✗                | Sin soporte confiable                       |

### 6.4 Cómo el proxy filtra el pool cuando hay tools
...
def get_eligible_models(pseudo_model: PseudoModel, session_caps: Capabilities) -> list[PhysicalModel]:
    ...
    if session_caps.has_tools:
        models = [m for m in models if m.openai_tools_compatible]
        if not models:
            raise ProxyError(
                "TOOLS_NO_ELIGIBLE_MODEL",
                f"El pseudo-modelo '{pseudo_model.name}' no tiene modelos con soporte "
                f"OpenAI-compatible de tools. Cambia a un pseudo-modelo que sí los tenga."
            )

    if session_caps.has_parallel_tools:
        parallel_eligible = [m for m in models if m.parallel_tools]
        if parallel_eligible:
            models = parallel_eligible
        # Si ninguno soporta parallel, se usa la lista filtrada con advertencia.
        # El historial paralelo puede corromperse — el proxy lo advierte.

    return models
```

Si el modelo físico fijado por afinidad no está en la lista elegible, el proxy:

1. Intenta el siguiente modelo de la lista elegible (respetando la prioridad configurada)
2. Actualiza la afinidad al nuevo modelo
3. Informa en `proxy_metadata`: `{"tools_filter_applied": true, "previous_model_excluded": "minimax-m2.5"}`
...
```json
{
  ...
  "capabilities": {
    ...
    "tools_eligible_models": ["deepseek-v3-pro", "deepseek-r1"],
    "total_tokens": 45230
  }
}
```
...
7. proxy_metadata: "images_described: 3, described_by: gemini-2.5-flash"
```
...
def validate_switch(
    ...
    if caps.has_parallel_tools:
        eligible = [m for m in to_pseudo.physical_models
                    if m.openai_tools_compatible and m.parallel_tools]
        if not eligible:
            return Blocked("PARALLEL_TOOLS_INCOMPATIBLE", remediation=[
                "POST /conversations/{id}/normalize-tools"
            ])
        if len(eligible) < len([m for m in to_pseudo.physical_models if m.openai_tools_compatible]):
            return Warning("PARALLEL_TOOLS_PARTIAL",
                "Algunos modelos del destino no soportan parallel calls.")

    # 3. Tools → modelo sin openai_tools_compatible
    if caps.has_tools:
        eligible = [m for m in to_pseudo.physical_models if m.openai_tools_compatible]
        if not eligible:
            return Blocked("TOOLS_NOT_SUPPORTED",
                f"El pseudo-modelo '{to_pseudo.name}' no tiene modelos OpenAI-compatible de tools.")

    # 4. Contexto acumulado → ventana insuficiente
    if caps.total_tokens > to_pseudo.context_window:
        return Blocked("CONTEXT_TOO_LARGE", remediation=[
            "POST /conversations/{id}/compact",
        ])

    # 5. Upgrade → siempre seguro
    return Safe()
```
...
| `normal`                    | `flash-lowcost`             | Sin tools                         | ⚠️ Advertencia | Pérdida significativa de capacidad             |
...
| `deep-flash`                | `normal`                    | Sin tools                         | ✅ Seguro      | —                                              |
| `deep-flash`                | `normal`                    | Con tools                         | ✅ Seguro      | Destino tiene mejor soporte                    |
...
sequenceDiagram
    ...
    participant C as Compactador (Gemini 2.5 Flash)
...
| 409  | `TOOLS_NOT_SUPPORTED`         | Pseudo-modelo destino no tiene modelos OpenAI-compatible de tools                              |
| 409  | `TOOLS_MODEL_INCOMPATIBLE`    | El modelo fijado no califica para tools. Proxy intentó fallback, también falló.                |
...
**Criterio de éxito:** Cambiar de `avanzada-vision` a `normal` con imágenes → HTTP 409 con opciones. MiniMax M2.5 excluido automáticamente cuando hay tools. `GET /compatible-models` es determinista.
...
**Criterio de éxito:** Un agente con parallel tools en `tareas-avanzadas` (DeepSeek V3 Pro) → `POST /normalize-tools` → migra a `normal` (Qwen) sin romper el historial. MiniMax nunca recibe tools.
...
**Criterio de éxito:** Una conversación de 2M tokens puede compactarse con Gemini 2.5 Flash (1M context) y reactivarse con un snapshot de ~10K tokens.
...
**Criterios:** `CONTEXT_UNUSABLE` incluye endpoint de compactación; compactador selecciona Gemini 2.5 Flash (1M context) automáticamente; snapshot resultante (~10K tokens) permite retomar inmediatamente.
...
flowchart TD
    ...
    K --> I
    I[Detectar capabilities del input] --> M{¿has_tools?}
    M -->|Sí| N[Filtrar pool: solo openai_tools_compatible]
    M -->|No| O
    N --> O{¿Input > umbral?}
...
```mermaid
flowchart TD
    A{¿has_tools?} -->|No| B[Solo reglas de multimedia]
    A -->|Sí| C[Filtrar pool por openai_tools_compatible]
    C --> D{¿Pool vacío?}
    D -->|Sí| E[HTTP 409 TOOLS_NOT_SUPPORTED\nSugerir pseudo-modelo con tools]
    D -->|No| F{¿has_parallel_tools?}
    F -->|No| G[Continuar con pool filtrado]
    F -->|Sí| H[Sub-filtrar: parallel_tools: true]
    H --> I{¿Sub-pool vacío?}
    I -->|Sí| J[⚠️ Warning: parallel no garantizado\nUsar pool completo filtrado]
    I -->|No| K[Usar sub-pool parallel]
    J --> L[Continuar]
    K --> L
    G --> L
```
...
```
pensamiento-profundo-caro:
  1. glm-5.1  →[503]→  2. o3  →[503]→  3. claude-opus-4  →[503]→  ERROR
  Con tools: solo modelos con openai_tools_compatible: true (todos los tres)
  Con parallel tools: solo o3 y claude-opus-4

tareas-avanzadas:
  1. deepseek-v3-pro  →[503]→  2. deepseek-r1  →[503]→  3. minimax-m2.5  →[503]→  ERROR
  Con tools: deepseek-v3-pro y deepseek-r1 (minimax excluido)
  Con parallel tools: deepseek-v3-pro y deepseek-r1

normal:
  1. qwen-3.6  →[503]→  2. deepseek-v3-flash  →[503]→  ERROR
  Con tools: ambos elegibles
  Con parallel tools: solo deepseek-v3-flash

deep-flash:
  1. glm-4.7-flash  →[503]→  2. groq-20b  →[503]→  3. deepseek-v3-flash  →[503]→  ERROR
  Con tools: todos elegibles (distintos niveles de confiabilidad)
  Con parallel tools: solo deepseek-v3-flash

flash-lowcost:
  1. glm-4.7-flash  →[503]→  2. qwen-3.5-9b  →[503]→  3. llama3.2 (local)  →[503]→  ERROR
  Con tools: glm-4.7-flash y qwen-3.5-9b (llama3.2 excluido)
  Con parallel tools: ninguno → bloqueo o normalización requerida

avanzada-vision:
  1. gemini-2.5-flash  →[503]→  2. minimax-m2.7-vision  →[503]→  ERROR
  Con tools: solo gemini-2.5-flash (minimax excluido)

flash-vision:
  1. gemini-2.0-flash  →[503]→  2. llava (local)  →[503]→  ERROR
  Con tools: solo gemini-2.0-flash (llava excluido)

compactador:
  Selecciona por context_window que cubra el historial dado:
  gemini-2.5-flash (1M)  →  claude-haiku-3-5 (200K)  →  glm-4.7-flash (128K)  →  ERROR
```
